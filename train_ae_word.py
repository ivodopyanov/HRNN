...
from Decoder.Decoder_Processor import Decoder_Processor
from Decoder.Decoder_RL_layer import Decoder_RL_Layer
...
def prepare_objects(data, settings):
    ...
    processor = build_processor(data, settings)
    predictor = build_predictor(data, settings)
    rl_model_encoder = build_RL_model_encoder(settings)
    rl_model_decoder = build_RL_model_decoder(settings)
    data_gen = build_generator_HRNN(data, settings, train_indexes)
    val_gen = build_generator_HRNN(data, settings, val_indexes)
    return {'processor': processor,
            'predictor': predictor,
            'rl_model_encoder': rl_model_encoder,
            'rl_model_decoder': rl_model_decoder,
            'data_gen': data_gen,
            'val_gen': val_gen,
