...
class Decoder(Layer):
    ...
    def vertical_step_action(self, prev_h_tm1, h_tm1, action_tm1, copy_prev_value_tm1):
        policy = activations.relu(K.dot(prev_h_tm1, self.W_action_1) + K.dot(h_tm1, self.U_action_1) + self.b_action_1)
        policy = K.minimum(K.exp(K.dot(policy, self.W_action_2)+self.b_action_2),1000)
        action = K.switch(TS.le(policy[:,0], policy[:, 1]), 1, 0)
        # Находим
        copy_prev_value = K.switch(copy_prev_value_tm1, 1, K.switch((1-action)*action_tm1, 1, 0))
        return action


    def vertical_step_h(self, prev_h, h_tm1, action):
