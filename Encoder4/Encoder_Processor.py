...
class Encoder_Processor(Encoder_Base):
    ...
    def vertical_step(self, x, x_mask, prev_has_value, prev_depth, bucket_size):
        ...
        next_x = K.concatenate([x[1:], K.zeros_like(x[0:1])], axis=0)

        results, _ = T.scan(self.horizontal_step,
                            sequences=[x, next_x, x_mask, prev_has_value, last_value_mask],
                            outputs_info=[initial_h, initial_new_mask, initial_has_value, initial_had_both_on_current_lebel])
        new_h = results[0]
        new_mask = results[1]
        has_value = results[2]
    ...
    def horizontal_step(self, x, next_x, prev_mask, prev_has_value, last_value_mask, h_tm1, new_mask_tm1, has_value_tm1, had_both_on_current_level_tm1):
        ...
        policy = activations.relu(K.dot(x*B_W, self.W_action_1) + K.dot(next_x*B_V, self.V_action_1) + K.dot(h_tm1*B_U, self.U_action_1) + self.b_action_1)
